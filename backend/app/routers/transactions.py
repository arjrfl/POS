...
@router.post("/{transaction_id}/void", dependencies=[Depends(require_role("admin"))])
async def void_transaction(
    ...
    try:
        transaction = await transaction_service.void_transaction(
            db, transaction_id, payload, current_user["user_id"]
        )
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc))
    except transaction_service.VoidPasswordError as exc:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=str(exc))
    except transaction_service.VoidValidationError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))
    return {"data": transaction, "error": None}
